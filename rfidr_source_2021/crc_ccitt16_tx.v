...
module    crc_ccitt16_tx
    ...
    always @(*) begin
        ...
        crc_out           =    ~shift_reg[15]; //As per Appendix F.2 of the EPC Gen 2 spec., we need to invert all these bits.
        
        if(initialize) begin
            shift_reg_next    =    16'hFFFF; //CRC_CCITT16 requires a register preload of all ones.
        end else if(shift) begin
            shift_reg_next    =    {shift_reg[14:0],1'b1}; //Shift out the bits when we need to append the CRC16 to the command.
        end else if(calculate) begin
            shift_reg_next    =    ({16{shift_reg[15] ^ bit_in}} & CRC_POLY) ^ {shift_reg[14:0],shift_reg[15] ^ bit_in};
            //Operate as an LFSR, computing the CRC16 as we process previous bits in the packet.
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            shift_reg    <=    16'hFFFF; //CRC_CCITT16 requires a register preload of all ones.
        end else begin
